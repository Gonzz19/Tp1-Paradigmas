...
class Magos : public Personaje, public std::enable_shared_from_this<Magos>
{
    protected:
        string nombre;
        int vida;
    ...
    public:
        Magos(string nombre, int vida, int ataque, int defensa_ad, int defensa_ap, int velocidad, bool bajo_efecto, int mana, int mana_max, int nivel, PERSONAJES tipo_personaje);
        ~Magos() = default;

        void set_mana(int m);
        int get_mana();     
        virtual void get_info() = 0;  
        virtual int get_ataque(unique_ptr<Arma> arma) override;
        int get_vida();
        string get_nombre();
        int getDefensa_ad();
        int getDefensa_ap();
